...
void print_table(std::ofstream& os, int col, int start, int end, std::function<int(int)> func) {
	int j = 10;
	for (int i=start; i < end; i++) {
		if (j >= 10) { os << std::endl; j = 1; }
		else j++;
		os << std::setw(col) << func(i);
	}
}
void output_rule_data()
{
	...
	output_file << "static constexpr int yylhs[] = {" << std::setw(42) << symbol_value[start_symbol] << ',';
	print_table(output_file, 5, 3, nrules, [&](int i)  { return symbol_value[rlhs[i]]; });
