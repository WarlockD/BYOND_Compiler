...
void FileError(const char *fmt, ...) {
  va_list args;
  std::cerr << (inc_file_name.empty() ? input_file_name.c_str() : inc_file_name.top().first.c_str()) << ':' << lineno << ':';
...
void error(int lineno, char *line, char *cptr, const char *msg, ...)
{
  char sbuf[512];
  va_list args;
