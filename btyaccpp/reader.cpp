...
std::unordered_map<string_t, string_t> defd_vars;
...
void check_symbols()
{
    ...
    if (goal->cclass == UNKNOWN)
	undefined_goal(goal->name);

    for (bp = first_symbol; bp; bp = bp->next) {
	if (bp->cclass == UNKNOWN) {
