...
void done(int k)
{
    if (action_file) { fclose(action_file); _unlink(action_file_name); }
    if (text_file) { fclose(text_file); _unlink(text_file_name); }
    if (union_file) { fclose(union_file); _unlink(union_file_name); }
    exit(k);
}
...
no_more_options:;
    ...
    if (!file_prefix) {
      if (input_file_name) {
	file_prefix = _strdup(input_file_name);
...
void create_file_names()
{
    ...
    if(_mktemp(action_file_name)==0) {
      fprintf(stderr, "btyacc: Cannot create temporary file\n");
      exit(1);
    }
    if(_mktemp(text_file_name)==0) {
      fprintf(stderr, "btyacc: Cannot create temporary file\n");
      exit(1);
    }
    if(_mktemp(union_file_name)==0) {
      fprintf(stderr, "btyacc: Cannot create temporary file\n");
      exit(1);
    }
